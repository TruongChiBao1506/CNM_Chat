...
class MeService {
  ...
  /**
   *
   * @param {*} _id  : id của user
   * @returns  Lấy danh sách danh bạ của người dùng và kiểm tra trạng thái bạn bè.
   */
  async getPhoneBooks(_id) {
    ...
    for (const userPhoneBookEle of phoneBooks) {
      ...
      try {
        const searchUser = await userService.getStatusFriendOfUser(_id, phone); // kiểm tra xem số điện thoại có thuộc một người dùng trong hệ thống không
   ...
   *
   * @param {*} _id : id của user
   * @param {*} phones Đồng bộ danh bạ người dùng với danh sách số điện thoại mới
   */
   ...
   * @param {*} source : nguồn của y định thu hồi token (web, mobile)
   * @returns Thu hồi tất cả token hiện tại của người dùng và tạo token mới
   */
  async revokeToken(_id, password, source) {
    await userValidate.validateEnterPassword(_id, password);

    // Xóa toàn bộ refreshTokens của người dùng và đặt timeRevokeToken là thời điểm hiện tại:
