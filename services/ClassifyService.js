...
class ClassifyService {
   ...
   * @returns : lấy danh sách phân loại của user
   */
  ...
  /**
   * Thêm conversation vào phân loại
   * @param {*} userId : id của user
   * @param {*} classifyId : id của phân loại
  ...
  async addConversation(userId, classifyId, conversationId) {
    ...
    // Kiểm tra conversation đã tồn tại trong phân loại chưa
    if (
    ...
    // Xóa conversation khỏi các phân loại khác
  ...
  async deleteConversation(userId, classifyId, conversationId) {
    ...
    if (nModified === 0) throw new NotFoundError("Delete Conversation Fail");
  }
}
