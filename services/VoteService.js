...
class VoteService {
  /**
   *
   * @param {*} conversationId : id của nhóm trò chuyện
   * @param {*} page : trang hiện tại
   * @param {*} size : số lượng tin nhắn trên một trang
   * @param {*} myId : id của người dùng
   * @returns  Lấy danh sách các tin nhắn bình chọn trong một nhóm trò chuyện.
   */
  ...
  async deleteVoteChoices(messageId, optionNames, userId) {
    ...
    message.options = message.options.map((optionEle) => {
      ...
      if (optionNames.includes(name) && index !== -1) userIds.splice(index, 1);
